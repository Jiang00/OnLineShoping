...
public class GridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private GridItemDecoration(int horizonSpan, int verticalSpan, int color, boolean showLastLine, int header) {
    ...
    private void drawHorizontal(Canvas c, RecyclerView parent) {
        ...
        for (int i = header; i < childCount; i++) {
            View child = parent.getChildAt(i);
            //最后一行底部横线不绘制
            if (isLastRaw(parent, i - header, getSpanCount(parent), childCount - header) && !mShowLastLine) {
                continue;
            }
    ...
    private void drawVertical(Canvas c, RecyclerView parent) {
        ...
        for (int i = header; i < childCount; i++) {
            ...
            if ((parent.getChildViewHolder(child).getAdapterPosition() + 1 - header) % getSpanCount(parent) == 0) {
                continue;
            }
...
//            //满足条件( 最后一行 && 不绘制 ) 将vertical多出的一部分去掉;
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int childCount = parent.getAdapter().getItemCount() - header;
        int itemPosition = ((RecyclerView.LayoutParams) view.getLayoutParams()).getViewLayoutPosition() - header;
        if (itemPosition < 0) {
            return;
        }
    ...
    public static class Builder {
        ...
        public GridItemDecoration build() {
            return new GridItemDecoration(mHorizonSpan, mVerticalSpan, mColor, mShowLastLine, hasHeader);
        }
    }
}